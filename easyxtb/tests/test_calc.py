...
def test_opt_neutral_singlet():
    for mol in ["acetic_acid", "acetone", "benzene"]:
        in_geom = Geometry.load_file(input_dir / f"{mol}.cjson")
        out_geom = calculate.optimize(in_geom)
        ref_geom = Geometry.load_file(results_dir / f"{mol}_opt.xyz")
        print(f"Testing {mol}:")
...
def test_freq_neutral_singlet():
    for mol in ["acetic_acid", "acetone", "benzene"]:
        opt_geom = Geometry.load_file(results_dir / f"{mol}_opt.xyz")
...
def test_freq_anion():
    for mol in ["benzoate"]:
        opt_geom = Geometry.load_file(results_dir / f"{mol}_opt.xyz", charge=-1)
        freqs = calculate.frequencies(opt_geom)
        with open(results_dir / f"{mol}_g98.out", encoding="utf-8") as f:
            ref_freqs = parse.parse_g98_frequencies(f.read())
        print(f"Testing {mol}:")
...
def test_freq_doublet():
    for mol in ["tempo"]:
        opt_geom = Geometry.load_file(results_dir / f"{mol}_opt.xyz", spin=1)
        freqs = calculate.frequencies(opt_geom)
        with open(results_dir / f"{mol}_g98.out", encoding="utf-8") as f:
            ref_freqs = parse.parse_g98_frequencies(f.read())
        print(f"Testing {mol}:")
