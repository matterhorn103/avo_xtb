...
"""Configuration of various options via the GUI."""
...
def update_config(avo_input: dict) -> dict:
    """Update the `easyxtb` config file based on the user's choices, and return the input CJSON
    unchanged."""
    ...
    if avo_input["user_dir"] != str(easyxtb.CALCS_DIR):
        ...
        try:
            ...
            output["error"] = "A folder could not be created at the path specified!"
